...
import "../../FacultyManagement/TeacherDashboard/DashboardCSS/Notification.css";

const InstituteNotifications = ({
  instituteDashboardDetails: initialData,
  user,
  token,
}) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const currentNotifications = notifications.filter(
          (n) =>
            n.notification_dtls_id !== notificationId &&
            n.notification_is_read !== true
        );
